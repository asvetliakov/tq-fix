...
i686-w64-mingw32-gcc -o build/selftest.exe test/selftest.c -I build/gen -O2 -Wall
...
TQFLICKER_LOG="$LOG_WIN" "$CX/bin/cxstart" --bottle "$(basename "$BOTTLE")" --no-convert \
...
for _ in $(seq 1 40); do [ -s "$WORK/report.txt" ] && break; sleep 0.25; done
